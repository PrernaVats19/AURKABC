...
# To identify aurkc Ensembl ID
# Load Ensembl dataset
mart <- useMart("ensembl", dataset = "hsapiens_gene_ensembl")

# Get Ensembl ID for aurkc
aurkc_info <- getBM(
...
#Extract aurkc Expression 
aurkc_expr <- expr_matrix[rownames(expr_matrix) == "ENSG00000105146", ]

#Remove Version Numbers
expr_matrix$gene <- sub("\\..*", "", expr_matrix$gene)
aurkc_expr <- expr_matrix %>% filter(gene == "ENSG00000105146")

#Extract AURKC Expression
aurkc_expr_long <- as.data.frame(t(aurkc_expr[-1]))  # Remove gene column
colnames(aurkc_expr_long) <- "aurkc_expression"
aurkc_expr_long$sample_id <- rownames(aurkc_expr_long)

#Transpose for Merging
clinical_data$sample_id <- rownames(clinical_data)  # Ensure matching sample IDs
final_data <- merge(aurkc_expr_long, clinical_data, by = "sample_id")
...
stat_test <- wilcox.test(aurkc_expression ~ sample_type, data = balanced_data)
...
ggplot(balanced_data, aes(x = sample_type, y = aurkc_expression, fill = sample_type)) +
  geom_boxplot(outlier.shape = NA, alpha = 0.7) +
  geom_jitter(shape = 16, position = position_jitter(0.2), alpha = 0.5) +
  scale_fill_manual(values = c("blue", "red")) +  # Adjust colors as needed
  labs(title = "Expression of AURKC in LUAD based on Sample Types",
       x = "TCGA Samples",
  ...
  annotate("text", x = 1, y = min(balanced_data$aurkc_expression) - 200, 
           label = "(n=59)", size = 5, fontface = "bold") +
  annotate("text", x = 2, y = min(balanced_data$aurkc_expression) - 200, 
           label = "(n=59)", size = 5, fontface = "bold") +
  
  # Add p-value above the plot
  annotate("text", x = 1.5, y = max(balanced_data$aurkc_expression) * 1.1, 
